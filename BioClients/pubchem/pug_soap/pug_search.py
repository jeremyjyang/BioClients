...
#############################################################################
def QueryPug_struct(smi,searchtype='sub',sim_cutoff=0.8,active=False,mlsmr=False,nmax=10000,verbose=0):
...
if __name__=='__main__':
  usage='''
  %(PROG)s - PubChem sub|sim|exact search, fetch smiles|sdfs

  required:
  --qsmi=<SMILES>   ... input query smiles (or smarts)
  --o=<OUTMOLS> ... output smiles|sdf file (w/ CIDs)
  --fmt=<FMT> ... smiles|sdf [default via filename]
  --gz           ... output gzipped [default via filename]
  --out_cids=<OUTCIDS> ... output CIDs (only w/ smiles output)
  --similarity   ... similarity search [default is substructure]
  --exact        ... exact search [default is substructure]
  --sim_cutoff=<tan>   ... similarity cutoff, Tanimoto [%(SIM_CUTOFF)f] 
...
'''%{'PROG':PROG,'SIM_CUTOFF':SIM_CUTOFF}
  ...
  qsmi=None; ofile=None; verbose=0;
  out_cids_file=None; searchtype='sub';
  active=False; mlsmr=False;
  fmt=None; gzip=False;
  opts,pargs=getopt.getopt(sys.argv[1:],'',['h','v','vv','qsmi=',
  'o=','out=','nmax=','n=','sim_cutoff=','similarity','sim','exact',
  'out_cids=','fmt=','gz','max_wait=','active','mlsmr'])
  if not opts: ErrorExit(usage)
  for (opt,val) in opts:
    if opt=='--h': ErrorExit(usage)
    elif opt=='--qsmi': qsmi=val
    elif opt in ('--o','--out'): ofile=val
    elif opt in ('--fmt','--format'): fmt=val
    elif opt=='--out_cids': out_cids_file=val
    elif opt in ('--similarity','--sim'): searchtype='sim'
    elif opt=='--exact': searchtype='exact'
  ...
  status,reqid,qkey,wenv,error=QueryPug_struct(qsmi,searchtype,SIM_CUTOFF,active,mlsmr,NMAX,verbose)
