...
def GetConcept(base_url, ver, src, auth, ids, skip, nmax, fout):
  ...
  for id_query in ids:
    ...
    vals = [csv_utils.ToStringForCSV(id_query)]
    for tag in result_tags:
      ...
      if tag == 'concepts':
        ...
        if type(val)==types.ListType:
          sts=[]
          for st in val:
            if type(st)==types.DictType and 'name' in st: sts.append(st['name'])
          val = ';'.join(sts)
	...
	'definitions'):
      ...
      vals.append(csv_utils.ToStringForCSV(val))
    ...
    while True:
      ...
      for atom in result:
        ...
        for tag in atom_tags:
          ...
          if type(val) in types.StringTypes:
            val = val.replace(base_url,'')
          if tag in ('relations', 'parents', 'children','descendants','ancestors',
	  'attributes', 'contentViewMemberships'):
